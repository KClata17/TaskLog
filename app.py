...
class Task_log(db.Model):
    ...
    user_id = db.Column(db.Integer, db.ForeignKey(
        'user_register.user_id'), nullable=False)
    ...
    categories = db.relationship(
        'task_catagories', backref='task', cascade="delete",  lazy=True)
...
class task_catagories(db.Model):
    ...
    task_id = db.Column(db.Integer, db.ForeignKey(
        'TaskLog.task_id'), nullable=False)
...
@app.route('/tasklog', methods=['POST', 'GET'])
@login_required
def tasklog():
    if request.method == "POST":
        ...
        # Prevent duplicate tasks (optional safety)
        existing_task = (
            db.session.query(Task_log)
            .join(task_catagories)
            .filter(
                Task_log.title == title,
                Task_log.description == description,
                Task_log.user_id == current_user.user_id,
                task_catagories.category_type == category_type
            ).first()
        )
        if existing_task:
            flash("This task already exists!", "warning")
            return redirect(url_for('tasklog'))

        task_todo = Task_log(
            title=title, description=description, user_id=current_user.user_id)
...
def mytask():
    user_tasks = (
        db.session.query(
            Task_log, task_catagories.category_type, user_register.name)
        .outerjoin(task_catagories, Task_log.task_id == task_catagories.task_id)
        .join(user_register, Task_log.user_id == user_register.user_id)
        .filter(Task_log.user_id == current_user.user_id)
        .all()
    )
    return render_template('tasklogdetail.html', alltasklog=user_tasks)
